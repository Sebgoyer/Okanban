...
},{"./cardsModule":2,"./listsModule":3,"./utils":6}],2:[function(require,module,exports){
const utils = require('./utils');
...
const cardsModule = {
    ...
    showAddCardModal(event) {
        //On récupère la liste parent la plus proche du bouton +
        let listElement = event.target.closest('.panel');
        //On récupère l'id stocké en data-list-id
        const listId = listElement.getAttribute('list-id');
        const modal = document.getElementById('addCardModal');
        //On assigne le listId en valeur du champ input caché dans le formulaire
...
const listsModule = {
    ...
    addListenerToActions() {
        //On affiche la modale d'ajout de liste au clic sur le bouton ajouter une liste
        const addListButton = document.getElementById('addListButton');
        addListButton.addEventListener('click', listsModule.showAddListModal);

        //Submit du formulaire d'ajout d'une liste
    ...
    showAddListModal() {
        ...
        //On ajoute la classe "is-active" (vient de Bulma) à la modale, ce qui l'affiche
        modal.classList.add('is-active');
    },
    ...
    async handleAddListForm(event) {
        ...
        try {
            ...
            if (response.status !== 200) {
                ...
                //Ajout dans le DOM de la nouvelle liste
    ...
    makeListInDOM(listName, listId) {
        ...
        //On copie ce template et on assigne la copie à la constante newList
        const newList = document.importNode(template.content, true);
        newList.querySelector('h2').textContent = listName;

        //On ajoute en data attribute cet id sur la liste
        newList.querySelector('.column').setAttribute('list-id', listId);
        //On met à jour la valeur de l'input caché avec listId
        newList.querySelector("form input[name='list-id']").value = listId;

        //On ajoute l'event listener sur le bouton + qui affiche la modale nouvelle carte
    ...
    //Récupération des listes depuis l'API et affichage dans le DOM
...
//On peut exporter comme en NodeJS !
module.exports = listsModule;
},{"./cardsModule":2,"./tagsModule":5,"./utils":6,"sortablejs":4}],4:[function(require,module,exports){
...
const tagsModule = {
    makeTagInDOM(tag) {
        //On récupère le template du tag
...
const utils = {
    ...
    hideModals() {
        //On enlève la classe "is-active" de toutes les modales
